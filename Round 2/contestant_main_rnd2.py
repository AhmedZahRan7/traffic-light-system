...
NUM_EPISODES = 5 # Number of complete simulation runs
...
if __name__ == "__main__":
    ...
    for max in range (3,10,1):
        for default in range (1,8,1):
            for min in range (0,3,1):
                avg = 0
                emission = 0
                agent = Agent(max,default,min)  # Instantiate your agent object
                ...
                for e in range(NUM_EPISODES):
                    ...
                    vehicles = gen_sim('', round=COMPETITION_ROUND,
                                    p_west_east=random.uniform(0.1,.9), p_east_west=random.uniform(0.1,.9),
                                    p_north_south=random.uniform(0.1,.9), p_south_north=random.uniform(0.1,.9))
                    ...
                    avg += avg_waiting_time
                    emission += avg_emissions
                    print(bcolors.FAIL + str(avg/(e+1)) + bcolors.ENDC)
                print(bcolors.HEADER + "max : {} default : {} min : {} avg is {} ,emission is {}\n".format(max,default,min, avg/NUM_EPISODES,emission/NUM_EPISODES)+ bcolors.ENDC)
                f.write("max : {} default : {} min : {} avg is {} ,emission is {}\n".format(max,default,min, avg/NUM_EPISODES,emission/NUM_EPISODES))